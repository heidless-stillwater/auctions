...
RSpec.describe BiddingEngine do
  describe ".bid!" do 
    ...
    let(:bidder) {
      User.create(:email => "john@doe.com", :password => "pw1234")
    }

    let(:auction) { Auction.create( title: 'Anything', \
    ...
    it "create a new bid on an auction if bid is bigger than last bid on auction" do
      ...
      expect(auction.errors[:bid].first).to eq "must be bigger than the last bid on this auction"
    end
